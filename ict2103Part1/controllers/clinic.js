...
router.get('/', function (req, res) {
    if (req.get("token") == null || req.get("token") == "") {
        ...
        common.checksession(db, token, function (returnValue) {
            if (returnValue) {
                db.query("select * from clinic", function (err, rows, fields) {
                    if (err) {
                        res.statusCode = 200
                        return res.json({
    ...
    if (req.get("token") == null || req.get("token") == "") {
        ...
        common.checksession(db, token, function (returnValue) {
            if (returnValue) {
                ...
                db.query("select * from clinic where ? ", parameters ,function (err, rows, fields) {
                    if (err) {
                        res.statusCode = 200
                        return res.json({
                            respond: "Database ran into some problem",
            ...
            if (returnValue) {
              if (req.body.searchvalue != "" && req.body.searchvalue != null) {
                var paremeter1 = '%' + req.body.searchvalue + '%'
                 db.query("SELECT * FROM clinic where ESTATE like ? OR CLINIC_NAME like ? OR POSTAL_CODE = ?;",[req.body.searchvalue,paremeter1,req.body.searchvalue] , function (err, rows, fields) {
                    if (err) {
                        res.statusCode = 200
                        return res.json({
                            respond: err,
